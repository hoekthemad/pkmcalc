let _import = {
    ...
    go: (data) => {
        ...
        if (false == data && undefined == localStorage.playerdata) {
            return;
        }
        else if (false == data && undefined != localStorage.playerdata) {
            _import.stats = JSON.parse(localStorage.playerdata);
    ...
    setData: (stats) => {
        try {
            ...
            //#endregion
            localStorage.setItem("playerdata", JSON.stringify(_import.stats));
            console.log(_import.stats);
        }
        catch (e) {
